<template>
  <div class="radar">
    <svg :width="size" :height="size" xmlns="http://www.w3.org/2000/svg" v-if="show">
      ...
      <g :transform="'rotate('+ -orientation +' '+ radius +' '+ radius +')'">
      ...
      <path d="m 0,0 -8,-8 q 8 -6 16 0 z" :transform="'translate('+ radius +' '+ radius +') scale(5)'" style="fill: rgba(227, 190, 88, 0.5)"/>
...
export default {
  ...
  computed: {
    ...
    mapProps () {
      ...
      return {
        transform: 'translate(' + (-xRatio + this.radius) + ',' + (-yRatio + this.radius) + ') rotate(-90 '+ xRatio  +' '+ yRatio +') rotate('+ -this.orientation +' '+ xRatio  +' '+ yRatio +')',
        height,
        width
      }
    }
